...
export class AppComponent implements OnInit {
  ...
  currentNodes: CurrentNodes = {};  // 현재 선택된 노드 정보를 담고 있는 Interface
  currentPath: string;
  ...
  // html에서 좌측 메뉴가 열려 있는지 여부
  get isOpened() {
    // 문서가 열려 있고 좌측메뉴가 열려있을때만 "True"
  ...
  ngOnInit(): void {
    ...
    // 일반적으로 새 문서에 대한 쉘 업데이트(예: 호스트 클래스, sidenav state)를
    // 종료한 후(새 문서에 대한 스타일을 조기에 적용하는 것을 피하기 위해)까지 지연시키고자 한다.
    // 그러나 첫 번째 문서의 경우(이전 문서가 없는 것을 알 경우),
    // 깜박이지 않도록 가능한 한 빨리 스타일을 적용하고 싶다.
    combineLatest(
  ...
  updateSideNav() {
    // 기본적으로 현재 sidenav 열기 상태를 유지합니다.
    let openSideNav = this.sidenav.opened;
    const isSideNavDoc = !!this.currentNodes[sideNavView]; // undefined or Null 일경우 false가 됨.
  ...
  // R엘레멘트가 커서 위에있을 때 엘리먼트 내부를 스크롤한다.
